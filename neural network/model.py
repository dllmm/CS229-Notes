...
class nn():
    ...
    def theta_init(self, x, y):
        theta = []
        theta.append(np.matrix(np.random.rand(self.neuron_num , x.shape[1] + 1) - 0.5 * np.ones([self.neuron_num , x.shape[1] + 1])) * 0.25)
        for i in range(self.hidden_num - 1):
            theta.append(np.matrix(np.random.rand(self.neuron_num , self.neuron_num + 1) - 0.5 * np.ones([self.neuron_num , self.neuron_num + 1]))*0.25)
        theta.append(np.matrix(np.random.rand(y.shape[1], self.neuron_num + 1) - 0.5 * np.ones([y.shape[1], self.neuron_num + 1]))*0.25)
        return theta
